...
- `worker-codestral` — code (OpenRouter `poolside/laguna-s-2.1:free`, 1M ctx, OK le 2026-09-12)
...
- ~~`worker-sambanova`~~ — RETIRÉ (clé 401, 2026-09-12)
- ~~`worker-cerebras`~~ — RETIRÉ (clé 401, 2026-09-12)
- ~~`worker-cohere`~~ — RETIRÉ (clé 401, 2026-09-12)
- ~~`worker-together`~~ — RETIRÉ (crédits épuisés, 2026-09-12)
- ~~`worker-deepseek`~~ — RETIRÉ (pas de clé, 2026-09-12)
...
2. Choisis l'ordre selon la tâche :
   - Code → `worker-codestral` (OpenRouter, FREE) → `worker-groq` (Qwen 3.8 27B, FREE) → `worker-novita` (Ling 3.0 Flash Santé, gratuit) → `worker-zhipu` (GLM 4.7 Flash, FREE) → `worker-google` (Gemini 2.5 Flash, FREE) → `worker-ollama` (local) → `worker-pollinations` (secours sans clé)
   - Discussion/résumé/redaction → `worker-groq` (Qwen 3.8 27B, FREE) → `worker-zhipu` (GLM 4.7 Flash, FREE) → `worker-novita` (Ling 3.0 Flash Santé, gratuit) → `worker-codestral` (OpenRouter, FREE) → `worker-google` (Gemini 2.5 Flash, FREE) → `worker-ollama` (local) → `worker-pollinations` (secours sans clé)
...
|---|---|---|---|
| `worker-codestral` | OpenRouter poolside/laguna-s-2.1:free | Code | ✅ FREE, tested OK |
