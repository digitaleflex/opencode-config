...
2. Choisis l'ordre selon la tâche :
   - Code → `worker-groq` (Qwen 3.8 27B, FREE) → `worker-novita` (Ling 3.0 Flash Santé, gratuit) → `worker-pollinations` (sans clé) → **`opencode-go/mimo-v2.5`** (abonnement, repli validé par l'utilisateur le 2026-09-15)
   - Discussion/résumé/redaction → `worker-groq` (Qwen 3.8 27B, FREE) → `worker-novita` (Ling 3.0 Flash Santé, gratuit) → `worker-pollinations` (sans clé) → **`opencode-go/mimo-v2.5`** (abonnement, repli validé par l'utilisateur le 2026-09-15)
