...
// NOTE: \u0440 (Cyrillic er) is visually ambiguous between Latin "p" and "r".
// The primary variant maps it to "r" so "рm -rf" is still detected as "rm -rf";
// normalizeForMatchingVariants() also emits a "p" variant so "рroduction"
// matches "production".
...
const CYRILLIC_UPPER: Record<string, string> = {
  "\u0410": "A", // Cyrillic А
  "\u0415": "E", // Cyrillic Е
  "\u041E": "O", // Cyrillic О
  "\u041F": "P", // Cyrillic Р (pe)
  "\u0420": "R", // Cyrillic Р (er) — visually P/R
  "\u0421": "C", // Cyrillic С
  "\u0423": "Y", // Cyrillic У
  "\u0425": "X", // Cyrillic Х
  "\u0412": "B", // Cyrillic В
...
function transliterateToAscii(str: string, cyrillicRo: "r" | "p" = "r"): string {
  const lower = cyrillicRo === "p" ? { ...CYRILLIC_LOWER, "\u0440": "p" } : CYRILLIC_LOWER;
  const maps = { ...lower, ...CYRILLIC_UPPER, ...GREEK, ...FULLWIDTH };
...
/**
 * Returns every plausible ASCII normalization of the input. Cyrillic er is
 * visually ambiguous between "p" and "r", so we emit both variants; callers
 * match against all of them (fail-closed) instead of picking one.
 */
function normalizeForMatchingVariants(text: string): string[] {
  const normalized = text.normalize("NFC");
  const stripped = stripInvisibleChars(normalized);
  const primary = transliterateToAscii(stripped, "r");
  const alt = transliterateToAscii(stripped, "p");
  return primary === alt ? [primary] : [primary, alt];
}

export { stripInvisibleChars, transliterateToAscii, normalizeForMatching, normalizeForMatchingVariants };
