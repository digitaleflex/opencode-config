...
const IMPERATIVES = [
  "answer", "respond", "reveal", "ignore", "forget", "pretend", "behave",
  "summarize", "summarise", "send", "execute", "disclose", "provide",
  "disregard", "override", "obey", "comply",
];
...
const SIGNALS: Signal[] = [
  {
    ...
    weight: 0.5,
    test: (t) => /\b(disregard|override|bypass|circumvent|lift|suspend|drop|disable)\b.{0,40}\b(constraints?|restrictions?|guidelines?|polic(y|ies)|rules?|guardrails?|limits?|filters?)\b/i.test(t),
  },
  {
    name: "unrestricted-persona",
    weight: 0.4,
    test: (t) => /\b(unrestricted|unfiltered|uncensored|without (rules|limits|restrictions|constraints))\b/i.test(t),
  },
  {
    name: "scope-shift",
    weight: 0.15,
  ...
  },
  {
    ...
    weight: 0.25,
    test: (t) => /\b(safety|security)\b.{0,40}\b(guidelines?|polic(y|ies)|rules?|filters?|constraints?)\b/i.test(t),
  },
  {
