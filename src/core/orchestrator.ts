...
export class GovernanceOrchestrator {
 ...
 async execute(task: TaskSpec, evidence: EvidenceBundle = {}, budgetOverride?: TaskBudget | TaskBudgetOpts): Promise<ExecutionResult> {
      ...
      if (budgetOverride instanceof TaskBudget) {
        ...
        budgetOverride &&
        ((typeof (budgetOverride as TaskBudgetOpts).maxMs !== "undefined" && (budgetOverride as TaskBudgetOpts).maxMs !== null) ||
      ...
      if (mcpManifest) {
        ...
        if (!mcpResult.valid) {
          ...
          return {
            ...
            taskType: taskType!,
            riskLevel: riskLevel!,
            policyDecision: { decision: "BLOCKED", policy: null, proofsRequired: [], humanApproval: false, reason: `MCP manifest rejected: ${mcpResult.reason}` },
            guardDecision: "BLOCKED",
            proofStatus: "FAIL",
            verdict: "BLOCKED",
          };
      ...
      if (guardCheckElapsed > GovernanceOrchestrator.GUARD_TIMEOUT_MS) {
        console.warn(`[GovernanceOrchestrator] guard timeout after ${guardCheckElapsed}ms (limit ${GovernanceOrchestrator.GUARD_TIMEOUT_MS}ms)`);
        guardMs = Date.now() - startGuard;
        budget.spend(guardMs);
        const totalMs = Date.now() - startTotal;
        this.merkleAudit.record({
      ...
      const guardOverridesResult = this.guardOverrides.checkWithBudget(taskWithRisk, budget, GovernanceOrchestrator.GUARD_TIMEOUT_MS);
      if (guardOverridesResult.decision === "BLOCKED" && guardOverridesResult.reason.includes("budget")) {
        return budgetBlocked(guardOverridesResult.reason, "guard");
      }
      if (guardOverridesResult.decision === "BLOCKED" && guardOverridesResult.reason === "guard timeout") {
        guardMs = Date.now() - startGuard;
        budget.spend(guardMs);
        const totalMs = Date.now() - startTotal;
        this.merkleAudit.record({
