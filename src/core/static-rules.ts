...
const BUNDLED_RULES: StaticRule[] = [
  // --- shell-exec ---
  {
    ...
    severity: "critical",
    pattern: "\\bexec\\s*\\(",
    description: "exec() dynamic execution",
  },
  {
    id: "SHELL-002",
    category: "shell-exec",
  ...
  },
  {
    ...
    category: "shell-exec",
    severity: "critical",
    pattern: "\\b(nc|ncat|netcat)\\b\\s+.*\\s+-e\\s+",
    flags: "i",
    description: "netcat with -e (reverse shell)",
  },
  // --- python-exec ---
  {
    id: "PY-001",
  ...
  },
  {
    ...
    description: "eval/exec dynamic code",
  },
  {
    id: "PY-004",
    category: "python-exec",
    severity: "high",
    pattern: "__import__\\s*\\(|\\bcompile\\s*\\(",
    description: "dynamic import/compile",
  },
  // --- js-exec ---
  {
    id: "JS-001",
    category: "js-exec",
    severity: "critical",
    pattern: "child_process\\s*\\.\\s*(exec|execSync|spawn|spawnSync)\\s*\\(",
  ...
  // --- crypto-mining ---
  {
    ...
    pattern: "stratum\\+tcp://",
    flags: "i",
    description: "mining pool stratum URL",
  },
  {
    id: "MIN-002",
    category: "crypto-mining",
    severity: "medium",
    pattern: "\\bxmrig\\b|\\bcgminer\\b|\\bethminer\\b",
    flags: "i",
  ...
  // --- network-exfil ---
  {
    ...
    severity: "high",
    pattern: "\\bexfiltrate\\b",
    flags: "i",
    description: "exfiltration keyword in code",
  },
  {
    id: "NET-002",
    category: "network-exfil",
    severity: "medium",
    pattern: "requests\\.(post|put)\\s*\\(\\s*['\"]https?://",
    flags: "i",
    description: "HTTP POST to URL in code",
  },
];
